...
- BUT, if the player rolls a 1 on either dice, all their ROUND score gets lost. After that, it's the next player's turn
...
document.querySelector('.btn-hold').addEventListener('click', function() {
    if (gamePlaying) {
        ...
        if (input) { 
            totalPointsNeeded = input;
        } else {
            totalPointsNeeded = 100;
        }

        // Check if player won the game
        if (scores[activePlayer] >= totalPointsNeeded) {
            document.querySelector('#name-' + activePlayer).textContent = 'Winner!';
            document.querySelector('#name-' + activePlayer).style.color = '#009900';
            hideDice();
